...
public class VerticalIndicatorView extends View implements Indicator {
    ...
    public VerticalIndicatorView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public VerticalIndicatorView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        Log.d("ZeOne","width=" + width + ",height=" + height + ",pagerCount=" + pagerCount + ",getRatioSelectedRadius=" + getRatioSelectedRadius() + ",getRatioRadius=" + getRatioRadius() + ",indicatorSpacing=" + indicatorSpacing);
...
//                result = (int) (diameterDistance + getPaddingLeft() + getPaddingRight());
...
//                float spacingDistance = (pagerCount - 1) * indicatorSpacing;
//                float al = ratioSelectedRadius - ratioRadius;
//                result = (int) (diameterDistance + spacingDistance + al + getPaddingTop() + getPaddingBottom());
//                break;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float midX = getWidth() / 2.0f + 0.5f;
        drawCircle(canvas, midX);
//        if (indicatorStyle == IndicatorStyle.INDICATOR_CIRCLE) {
//            drawCircle(canvas, midX);
//        } else if (indicatorStyle == IndicatorStyle.INDICATOR_CIRCLE_RECT) {
//            drawCircleRect(canvas, midX);
//        } else if (indicatorStyle == IndicatorStyle.INDICATOR_BEZIER) {
//            drawBezier(canvas, midX);
//        } else if (indicatorStyle == IndicatorStyle.INDICATOR_DASH) {
//            drawDash(canvas, midX);
//        } else if (indicatorStyle == IndicatorStyle.INDICATOR_BIG_CIRCLE) {
//            drawBigCircle(canvas, midX);
//        }
    }

    private void drawCircle(Canvas canvas, float midY) {
        drawPagerCountCircle(canvas, midY);
        float indicatorStartY = indicatorStartY(selectedPage);
        float nextIndicatorStartY = indicatorStartY((selectedPage + 1) % pagerCount);
        float ratioRadius = getRatioSelectedRadius();
        float top = indicatorStartY - ratioRadius;
        float bottom = indicatorStartY + ratioRadius;
        float nextTop = nextIndicatorStartY - ratioRadius;
        float nextBottom = nextIndicatorStartY + ratioRadius;
        float topX = top + (nextTop - top) * interpolatedOffset();
        float bottomX = bottom + (nextBottom - bottom) * interpolatedOffset();
        rectF.set(midY - indicatorSelectedRadius, topX, midY + indicatorSelectedRadius, bottomX);
    ...
    private void drawPagerCountCircle(Canvas canvas, float midY) {
        indicatorPaint.setColor(unColor);
        Log.d("ZeDraw","midY=" + midY + ",indicatorRadius=" + indicatorRadius + ",getRatioRadius=" + getRatioRadius());
        for (int i = 0; i < pagerCount; i++) {
            float startCy = indicatorStartY(i);
            Log.d("ZeOne","startCy=" + startCy);
            float ratioIndicatorRadius = getRatioRadius();
            float top = startCy - ratioIndicatorRadius;
            float left = midY - indicatorRadius;
            float bottom = startCy + ratioIndicatorRadius;
            float right = midY + indicatorRadius;
            rectF.set(left, top, right, bottom);
            canvas.drawRoundRect(rectF, indicatorRadius, indicatorRadius, indicatorPaint);
        }
    }
    ...
    private float indicatorStartY(int index) {
        float ratioRadius = getRatioRadius();
        float ratioSelectedRadius = getRatioSelectedRadius();
        float ratioIndicatorRadius = Math.max(ratioRadius, ratioSelectedRadius);
        float centerSpacing = ratioIndicatorRadius * 2.0f + indicatorSpacing;
        float centerX = ratioIndicatorRadius + getPaddingTop() + centerSpacing * index;
        /*
           为了适配INDICATOR_DASH样式， measure 中默认多增加了 ratioIndicatorRadius - ratioRadius 的高度和宽度
           除了INDICATOR_DASH样式下，其他样式需要增加indicatorSelectedRadius一半的距离，让位置居中。
         */
     ...
     */
    public VerticalIndicatorView setIndicatorRadius(float indicatorRadius) {
     ...
     * {@link VerticalIndicatorView#getRatioRadius()}
     *
     * @param indicatorRatio indicatorRadius * indicatorRatio
     */
    public VerticalIndicatorView setIndicatorRatio(float indicatorRatio) {
     ...
     */
    public VerticalIndicatorView setIndicatorSelectedRadius(float indicatorSelectedRadius) {
     ...
     */
    public VerticalIndicatorView setIndicatorSelectedRatio(float indicatorSelectedRatio) {
     ...
     */
    public VerticalIndicatorView setIndicatorSpacing(float indicatorSpacing) {
        this.indicatorSpacing = dip2px(indicatorSpacing);
        return this;
    }

    public VerticalIndicatorView setIndicatorStyle(@IndicatorStyle int indicatorStyle) {
        this.indicatorStyle = indicatorStyle;
        return this;
    }

    public VerticalIndicatorView setIndicatorColor(@ColorInt int indicatorColor) {
        this.unColor = indicatorColor;
        return this;
    }

    public VerticalIndicatorView setIndicatorSelectorColor(@ColorInt int indicatorSelectorColor) {
        this.selectedColor = indicatorSelectorColor;
        return this;
    }

    public VerticalIndicatorView setParams(RelativeLayout.LayoutParams params) {
        this.params = params;
        return this;
    }
}
